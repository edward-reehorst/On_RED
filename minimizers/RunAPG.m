...
%   input and sig_f is the input noise std deviation for the denoiser
...
%               use_fft        : If true, use fast prox via fft, if false,
%                               use an inner loop to solve prox
%               psf            : the Point Spread Function (used only when 
%                               use_fft == true). 
%               inner_iters    : If use_fft = false, this controls the
%                               number of iterations to solve the prox
%               lambda         : Regularization parameter
%               L              : Proximal Gradient, penalty parameter
%               effective_sigma: noise level used by the denoiser
...
function [ sol, outs, opts ] = RunAPG(y, ForwardFunc, BackwardFunc,...
    InitEstFunc, f_denoiser, input_sigma, opts, orig_im )
...
x0 = InitEstFunc(y);
 ...
 % Define instant functions for f, gradf, g and proxg
 f = @(x,fx) opts.lambda/2*sum(x(:).*(x(:)-fx(:)));
 gradf = @(x,fx) opts.lambda*(x-fx);
 
 % fill opts
 opts = setDefaults(opts,x0,gradf); % fill default values for options
 
 % Define instant functions for g and proxg
 g = @(x) 0.5*input_sigma^-2*norm(ForwardFunc(x)-y,'fro')^2;
 if isfield(opts,'use_fft') && opts.use_fft
    [h, w, ~] = size(y);
    fft_psf = zeros(h, w);
    t = floor(size(opts.psf, 1)/2);
    fft_psf(h/2+1-t:h/2+1+t, w/2+1-t:w/2+1+t) = opts.psf;
 ...
 psnrValues = zeros(max_iters,1);     %  PSNR
 a_time = zeros(max_iters,1);           % array to store time at every iteration
 ...
 t_start = tic;   % Begin recording solve time
 x1       = x0;
 den1     = f_denoiser(x1,opts.effective_sigma);
 f1       = f(x1,den1);
 fVals(1) = f1;
 gradf1   = gradf(x1,den1);
 ...
 for i = 1:max_iters
     ...
     fVals(i) = f1;
     psnrValues(i) = psnr(x1,orig_im,255);
     funcValues(i) = opts.function(x0);
     a_time(i) = toc(t_start);
     if opts.recordObjective   %  Record function values
    ...
    %  If we stop, then record information in the output struct
     if opts.stopNow(x1,i,residual(i),normalizedResid(i),maxResidual,opts) || i>=max_iters
         ...
         outs.psnrValues = psnrValues(1:i);
         outs.L = opts.L;
         outs.initialStepsize = opts.tau;
         outs.iterationCount = i;
         outs.a_time = a_time(1:i);
         if ~opts.recordObjective
         ...
         if opts.verbose
             fprintf('%s\tDone:  time = %0.3f secs, iterations = %i\n',opts.stringHeader, toc(t_start), outs.iterationCount);
         end
         return;
     end
...
function opts = setDefaults(opts,x0,gradf)
...
if (~isfield(opts,'L') || opts.L<=0) && (~isfield(opts,'tau') || opts.tau<=0)
    ...
    x2 = randn(size(x0));
    den1 = f_denoiser(x1,opts.effective_sigma);
    den2 = f_denoiser(x2,opts.effective_sigma);
    gradf1 = gradf(x1,den1);
    gradf2 = gradf(x2,den2);
...
function x_hat = proxg_fft(v,tau, fft_Ht_y, fft_HtH)
    % Use fft
    b = fft_Ht_y + 1/tau*fft2(v);
    A = fft_HtH + 1/tau;
    x_hat = real(ifft2( b./A ));
    x_hat = max( min(x_hat, 255), 0);
...
function x_hat = proxg_nonfft(v,tau,input_sigma,Ht_y, opts)
    for i = 1:opts.inner_iters
        ...
        res = b - ht_h_x_est - 1/tau*x_hat;
        A_res = BackwardFunc(ForwardFunc(res))/(input_sigma^2) + 1/tau*res;
        mu_opt = mean(res(:).*A_res(:))/mean(A_res(:).*A_res(:));
        x_hat = x_hat + mu_opt*res;
        x_hat = max( min(x_hat, 255), 0);
    end
return
